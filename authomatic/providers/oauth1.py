...
def _normalize_params(params):
    ...
    params = sorted(
        [(k, v) for k, v in params if k not in ("oauth_signature", "realm")]
    )
    # sort
...
class OAuth1(providers.AuthorizationProvider):
    ...
    def create_request_elements(
        ...
        if request_type == cls.USER_AUTHORIZATION_REQUEST_TYPE:
            ...
            params["oauth_signature"] = (
                cls._signature_generator.create_signature(  # noqa
    ...
    def login(self):
        ...
        if request_token and verifier:
            ...
            # Get Access Token
            self._log(
                logging.INFO, f"Fetching for access token from {self.access_token_url}."
            )
...
class Bitbucket(OAuth1):
    ...
    * timezone
    ...
    def _x_user_parser(user, data):
        ...
        user.link = "https://bitbucket.org/api{0}".format(_user.get("resource_uri"))
        return user
...
class Vimeo(OAuth1):
    ...
    * username
    ...
    supported_user_attributes = core.SupportedUserAttributes(
        id=True, link=True, location=True, name=True, picture=True
    )
...
class Xero(OAuth1):
    ...
    * username
    ...
    supported_user_attributes = core.SupportedUserAttributes(
        email=True, first_name=True, id=True, last_name=True, name=True
    )
    ...
    def _x_user_parser(user, data):
        ...
        if _users and _users[0]:
            ...
            if _address:
                ...
                user.phone = (
                    _address.get("phone", "") or _address.get("mobile_phone", "")
                ).replace("|", "")

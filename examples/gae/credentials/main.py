    ...
    def get(self):
        ...
        if error:
            ...
            if serialized_credentials:
                ...
                """.format(
                        dict(fb="Facebook", tw="Twitter")[credentials.provider_name]
                    )
                )

                valid = "still" if credentials.valid else "not anymore"
                expire_soon = (
                    "less" if credentials.expire_soon(60 * 60 * 24) else "more"
                )
    ...
    def post(self, provider_name):
        ...
        if provider_name == "fb":
            ...
            response = authomatic.access(
                serialized_credentials, url, params=dict(message=message), method="POST"
            )
